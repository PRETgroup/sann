...
// mathematical hyperbolic cosine function
fix16_t sech_f(fix16_t sum)
{
	return fix16_div(fix16_from_int(2), fix16_add(fix16_exp(sum), fix16_exp(-sum)));
}

fix16_t tanh_f(fix16_t sum)
{
	fix16_t numerator = fix16_sub(fix16_from_int(1), fix16_exp(fix16_mul(fix16_from_int(-2), sum)));
	fix16_t denominator = fix16_add(fix16_from_int(1), fix16_exp(fix16_mul(fix16_from_int(-2), sum)));
	return fix16_div(numerator, denominator);
}
...
// update the ANN once based off of one set of data
// used in online learning when learning is period based
void ann_train_online_once(ANN_F *ann, int index, fix16_t trainData[][2 * MAX_NEURONS], int num_epochs)
{
	// training only variables
	int i, j, k, weighted_sum;
	int num_outputs = ann->layers[ann->num_layers - 1];
	int num_weights = ann->num_layers - 1;
	...
	// printf("EPOCH: %d		INDEX: %d		LEARNING RATE: %f\n", num_epochs, index, fix16_to_float(lr));
	//printf("ANN -> Current learning rate is %f\n", lr);
...
#pragma loopbound min 0 max MAX_WEIGHTS_BIAS
	...
	fix16_t inputs[ann->layers[0]];
	fix16_t outputs[ann->layers[ann->num_layers - 1]];
#pragma loopbound min 0 max MAX_NEURONS
	for(j = 0; j < ann->layers[0]; j++) // reading inputs
	{
		inputs[j] = trainData[index][j];
		//printf("%d[%d]: %d\n", i, j, data.inputs[i][j]);
	}
	//printf("OUTPUTS:\n");
#pragma loopbound min 0 max MAX_NEURONS
	for(j = 0; j < num_outputs; j++) // reading outputs
	{
		outputs[j] = trainData[index][j + ann->layers[0]];
		//printf("%d[%d]: %d\n", i, j, data.outputs[i][j]);
	}

	fix16_t result[num_outputs]; // stores the result of the ann_run
	RUN_ANN_FIXED(inputs, result, ann, i, j, k, weighted_sum, MAX_NEURONS, MAX_LAYERS_LESS1)  // run ANN with selected inputs

	// get weight update for this input/output pair
	ann_get_deltas_fixed(ann, result, outputs, ann->max_weights, delta_weights, lr, 1);
...
// online training function for fixed point neural networks
// start selects where in the data set the algorithm begins training from
// finish selects at which data point the training stops
void ann_train_online_fixed(ANN_F *ann, int start, int finish, fix16_t trainData[][2 * MAX_NEURONS], int epochs, float error, int debug)
{
	int i = 0;
	int j = 0;
	int k = 0;
...
#pragma loopbound min 0 max MAX_NEURONS
		for(j = 0; j < ann->layers[0]; j++) // reading inputs
		{
			data.inputs[i][j] = trainData[i + start][j];
			//printf("%d[%d]: %d\n", i, j, data.inputs[i][j]);
		}
		//printf("OUTPUTS:\n");
#pragma loopbound min 0 max MAX_NEURONS
		for(j = 0; j < num_outputs; j++) // reading outputs
		{
			data.outputs[i][j] = trainData[i + start][j + ann->layers[0]];
			//printf("%d[%d]: %d\n", i, j, data.outputs[i][j]);
		}
	}
...
//						printf("ANN -> Delta sum for previous neuron is currently %f\n", delta_sums[layer - 2][j]);
				...
				case 1: // differentiate tanh(x) = f(x): f'(x) = sech(x)^2 = 1/cosh(x)^2
					temp = sech_f(ann->sums[layer][i]);
					error_gradient = fix16_mul(temp, temp);
					// delta_sum = (1/cosh(sum))^2 * output_error (ann->sums starts at 1, not 0)
