...
void __attribute__((noinline)) run_XOR_ANN_i1(float x)
{
	netw.Neurons[0].InputValue = x;

	for(uint32_t i=LAYERS_INPUT_LEN; i < NUM_NEURONS; i++) {
		netw.Neurons[i].InputValue = 0;
	}
}
void __attribute__((noinline)) run_XOR_ANN_i2(float y)
{
	netw.Neurons[1].InputValue =y;
}

void __attribute__((noinline)) run_XOR_ANN_h1()
{
...
void __attribute__((noinline)) run_XOR_ANN_h2()
{
#pragma loopbound min 0 max 2
	for(int i = 1; i < 5; i+=3)
	{
...
void __attribute__((noinline)) run_XOR_ANN_h3()
{
#pragma loopbound min 0 max 2
	for(int i = 2; i < 6; i+=3)
	{
...
float __attribute__((noinline)) run_XOR_ANN_o1()
{
	RunNetwork_l3();
	return ThresholdFunction(netw.Neurons[NUM_NEURONS-1].InputValue);
}
